...
Part B is based on inversionModel function (defined in LDmixtureModel.R). This function has as input the genotypes for a SNP-block pair. Therefore, this matrix has two columns, corresponding to the two SNP blocks, and the rows are the chromosomes. inversionModel implements a Expectation-Maximization algorithm to estimate the parameters of the LDmixture model (see paper). This function and the functions to compute the recomb and linkage models (defined in recombFreq.R -recomb model- and linkageFreq.R -linkage model-) are implemented in R, so they are very slow. All these functions might be implemented in C or C++, which will result in a great reduction in computation time. 
